...
<script>
  ...
  import { Search, Package, ChevronRight, AlertCircle } from 'lucide-svelte';
  ...
  async function viewProductTree(productId) {
    try {
      ...
      <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {#each products as product (product.id)}
            <div
              class="p-4 border border-gray-200 rounded-lg hover:border-blue-400 hover:shadow-md transition cursor-pointer"
              on:click={() => viewProductTree(product.id)}
              on:keypress={(e) => e.key === 'Enter' && viewProductTree(product.id)}
              role="button"
              tabindex="0"
