...
export const sendWelcomeEmail = async (email: string) => {
	const url = BASE_URL + resolve(`/auth/sign-in`);
...
export const updateEmailAddressSuccessEmail = async (
	...
	const textEmailChange = `L'adresse email de votre compte ${APP_NAME} a été modifiée de ${oldEmail} à ${email}. Si vous N'AVEZ PAS demandé cette modification, veuillez contacter le support à : `+url+` pour annuler les changements.`;
